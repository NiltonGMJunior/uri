...
int main()
{
    ...
    while (true)
    {
        ...
        for (unsigned short int iii = 0; iii < n; ++iii) std::cin >> marbles[iii];
        ...
        for (unsigned short int _ = 0; _ < q; ++_)
        {
            ...
            while (first <= last && index == -1)
            {
                ...
                mid = (first + last) / 2;
                // Found the marble
                if (marbles[mid] == query) index = mid;
                // Maybe marble is somewhere before the mid position
                else if (marbles[mid] > query) last = mid - 1;
            ...
            else
            {
                ...
                while (index > 0 && marbles[index - 1] == marbles[index]) index -= 1;
